...
static int s3c2440camif_deallocate_frame_buf(s3c2440camif_dev * cam, int count)
{
	...
	for (i = 0; i < count; i++) {
		if(cam->frame[i].virt_base)
			free_pages(cam->frame[i].virt_base, cam->frame[i].order);
			//dma_free_coherent(0,PAGE_ALIGN(cam->v2f.fmt.pix.sizeimage),cam->frame[i].virt_base,cam->frame[i].phy_base);
		cam->frame[i].virt_base=NULL;
...
static int s3c2440camif_allocate_frame_buf(s3c2440camif_dev * cam, int count)
{
  ...
  //printk(KERN_ALERT"s3c2440camif: Allocating %d buffers, %d each\n",count,cam->v2f.fmt.pix.sizeimage);
	
	for (i = 0; i < count; i++) {
		if(cam->frame[i].virt_base)
			printk(KERN_ERR "s3c2440camif:allocate_frame_buf called without freeing old buffers!.\n");
		
		cam->frame[i].order = get_order(cam->v2f.fmt.pix.sizeimage);
		cam->frame[i].virt_base = __get_free_pages(GFP_KERNEL|GFP_DMA, cam->frame[i].order);
		
  	if (cam->frame[i].virt_base == 0) {
...
static void __inline__ update_target_fmt_regs(s3c2440camif_dev * pdev)
{
  ...
  //setup for YCrCb422 mode
  //TODO: add swithc for other modes
  Ysize=   pdev->v2f.fmt.pix.width*pdev->v2f.fmt.pix.height;
  CbCrsize=pdev->v2f.fmt.pix.width*pdev->v2f.fmt.pix.height/2;
  
	
  //TODO: convert to proper use of video4linux buffers
  iowrite32(virt_to_bus(pdev->frame[0].virt_base), S3C244X_CICOYSA1);
  iowrite32(virt_to_bus(pdev->frame[1].virt_base), S3C244X_CICOYSA2);
  iowrite32(virt_to_bus(pdev->frame[2].virt_base), S3C244X_CICOYSA3);
  iowrite32(virt_to_bus(pdev->frame[3].virt_base), S3C244X_CICOYSA4);

  iowrite32(virt_to_bus(pdev->frame[0].virt_base+Ysize), S3C244X_CICOCBSA1);
  iowrite32(virt_to_bus(pdev->frame[1].virt_base+Ysize), S3C244X_CICOCBSA2);
  iowrite32(virt_to_bus(pdev->frame[2].virt_base+Ysize), S3C244X_CICOCBSA3);
  iowrite32(virt_to_bus(pdev->frame[3].virt_base+Ysize), S3C244X_CICOCBSA4);

  iowrite32(virt_to_bus(pdev->frame[0].virt_base+Ysize+CbCrsize), S3C244X_CICOCRSA1);
  iowrite32(virt_to_bus(pdev->frame[1].virt_base+Ysize+CbCrsize), S3C244X_CICOCRSA2);
  iowrite32(virt_to_bus(pdev->frame[2].virt_base+Ysize+CbCrsize), S3C244X_CICOCRSA3);
  iowrite32(virt_to_bus(pdev->frame[3].virt_base+Ysize+CbCrsize), S3C244X_CICOCRSA4);
  #endif
...
static void __inline__ update_target_zoom_regs(s3c2440camif_dev * pdev)
{
    ...
    iowrite32(ciprscpreratio, S3C244X_CIPRSCPRERATIO);
	  ...
	  pdev->bypass_mode=(pdev->v2f.fmt.pix.width==pdev->srcHsize && pdev->v2f.fmt.pix.height==pdev->srcHsize );
    /* CIPRSCPRERATIO. */
...
static ssize_t camif_read(struct file *file, char __user *data, size_t count, loff_t *ppos)
{
 ...
 //try to find a valid frame
  //if( last_frame != -1)
	...
	copy_to_user(data, (void *)pcam->frame[last_frame].virt_base, count);
...
static int s3c2410camif_set_format(s3c2440camif_dev *pcam,u32 format,u32 width,u32 height)
{
  ...
  int last_state=pcam->state;
	...
	if(width >pcam->srcHsize) width=pcam->srcHsize;
	if(height>pcam->srcVsize) height=pcam->srcVsize;
	
	//round up to an integer scaling factor
	scH=pcam->srcHsize/width;
	scV=pcam->srcVsize/height;
	
	width=pcam->srcHsize/scH;
	height=pcam->srcVsize/scV;
...
extern s3c2440camif_sensor s3c2440camif_sensor_if;
...
static int init_s3c2410camif_struct(s3c2440camif_dev * cam)
{
  ...
  cam->video_dev = video_device_alloc();
	...
	for (i = 0; i < S3C2440_FRAME_NUM; i++) {
		//cam->frame[i].phy_base  =0;
		cam->frame[i].virt_base =0;
	}
	
	cam->cam_sensor = &s3c2440camif_sensor_if;
...
/*	cam->overlay_on = false;
	...
	cam->srcHsize = 1280;	// FIXME, the OV9650's horizontal output pixels.
	cam->srcVsize = 1024;	// FIXME, the OV9650's verical output pixels.
...
static int __init camif_init(void)
{
  ...
  if((ret=init_s3c2410camif_struct(cam))<0)
	...
	if (err != 0) {
		pr_debug("camif_init: platform_device_register failed.\n");
